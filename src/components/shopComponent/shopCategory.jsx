...
const ShopCategory = () => {
    ...
    const fetchShopData = async () => {
      try {
        const response = await axios.get(
          'https://laravel-backend-production-b92c.up.railway.app/api/shop/data/fetch'
        );
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(
        'https://laravel-backend-production-b92c.up.railway.app/api/shop/product/fetch',
        {
          params: params,
        }
      );
  ...
  {products.map((product) => (
    ...
    >
      <div className='w-full h-36 sm:h-44 md:h-52 overflow-hidden'>
        <img
          src={`https://laravel-backend-production-b92c.up.railway.app/${product.image}`}
          alt={product.name}
  ...
  {placeholders.map((_, index) => (
    ...
    >
      ...
      <div className='flex justify-center mt-4'>
        ...
        <span className='px-4 py-2 m-2 border border-lime-500 rounded'>
          {page}
        </span>
        <button
          onClick={() => handlePageChange(page + 1)}
