...
const Products = () => {
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: `url('/Images/greeny.jpg')`,
          ...
          <div className='flex space-x-2 md:space-x-4 flex-wrap justify-center md:justify-end'>
            <button
              className={`px-2 py-1 sm:px-4 sm:py-2 rounded-full ${
                currentCategory === 'all'
                  ? 'bg-gray-400 text-white text-sm sm:text-lg'
                  : 'bg-amber-500 text-gray-700 text-sm sm:text-lg'
              }`}
            ...
            >
              <div className='w-full h-36 sm:h-44 md:h-52 overflow-hidden'>
                <img
                  src={product.image}
                  alt={product.name}
                  className='w-full h-full object-cover transition duration-300 zoom-in transform hover:scale-105'
                />
              </div>
